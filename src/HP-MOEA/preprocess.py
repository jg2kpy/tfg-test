...
def main(dataset_path = '../../datasets/ml-100k/ratings.csv', output_path = './data', top_usuarios = 10000, new_comer_filter = 5, dataset_100k_item_info_path = '../../datasets/ml-100k/u.item.csv'):
    ...
    for movie in movies_info.get_all_movies():
        ...
        binary_number = int("".join(map(str, binary_array)), 2)  # Convert binary array to integer
        movie.set_ADN(binary_number)

    for user in users2movie_ratings.get_all_users():
        top_movies = sorted(user.get_movies_and_ratings(), key=lambda x: x[1], reverse=True)[:5]
        movie_details = []
        for movie_id, _ in top_movies:
            movie_details.append(movies_info.get_movie_by_id(movie_id).get_ADN())
        binary_or_result = 0
